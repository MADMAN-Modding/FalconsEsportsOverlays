...
setInterval(function () {
   fetch('../json/overlay.json?' + i)
        ...
        .then((jsonData) => {
            ...
            for (let i = 1; i <= 4; i++) {
                document.getElementById(scoreArray[i-1] + "Left").style.stroke = winsLeft >= i ? "#FFF" : "#000";
            }

            document.getElementById("circleLeft").style.fill = winsLeft == 5 ? "white" : "black";

            // Update the right side rounds won
            for (let i = 1; i <= 4; i++) {
                document.getElementById(scoreArray[i-1] + "Right").style.stroke = winsRight >= i ? "#FFF" : "#000";
            }

            document.getElementById("circleRight").style.fill = winsRight == 5 ? "white" : "black";
            
            // Sets all the home team respective colors
            let teamColorLeft = jsonData["teamColorLeft"];
            changeBackgroundColor("cardRight", teamColorLeft);

            // Set the away team respective colors
            let teamColorRight = jsonData["teamColorRight"];
            changeBackgroundColor("cardLeft", teamColorRight);
